      ...
      if (existing) {
        ...
        if (!supported) {
          ...
          if (sub) {
            ...
            alert("No se pudo activar. Verificá que la app está instalada como PWA y volvá a intentar.");
          }
    ...
    <>
      <section className="page">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {editName ? (
              <form onSubmit={saveName} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                />
                <button type="submit" className="primary" style={{ padding: "8px 12px", fontSize: 13, borderRadius: 10 }}>OK</button>
                <button type="button" className="ghost" style={{ padding: "8px 12px", fontSize: 13, borderRadius: 10 }} onClick={() => setEditName(false)}>Cancelar</button>
              </form>
            ) : (
            ...
            {nameMsg && <small style={{ color: nameMsg.startsWith("Nombre") ? "var(--green)" : "var(--danger)" }}>{nameMsg}</small>}
        ...
        {role === "user" && (() => {
          ...
          const daysUntilRenewal = subExpiresAt ? Math.max(0, Math.ceil((subExpiresAt - Date.now()) / (24*60*60*1000))) : null;
          ...
          return (
            <div style={{ background: isActive ? "rgba(52,211,153,.08)" : "rgba(168,85,247,.06)", border: `1px solid ${isActive ? "rgba(52,211,153,.25)" : "rgba(168,85,247,.2)"}`, borderRadius: 16, padding: "14px 16px", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: isActive ? 8 : 0 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: isActive ? "var(--green)" : "var(--muted)" }}>{isActive ? "Activa" : "Inactiva"}</span>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--muted)" }}>
                    {isActive
                      ...
                      : "Accedá al Coach IA y Nutrición — $10.000/mes"}
                  </div>
                </div>
              </div>
              {!isActive && (
          ...
          return (
            <div className="card" style={{ background: "linear-gradient(135deg, rgba(168,85,247,.1) 0%, rgba(52,211,153,.08) 100%)", border: "1px solid rgba(168,85,247,.25)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "var(--accent)" }}>Referidos</span>
                <div>
        ...
        <div className="card">
              ...
              {[
                ...
                return (
                  <button key={opt.id} onClick={async () => {
                    ...
                    {isActive && <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--green)", fontWeight: 900 }}>activo</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="settings-row" style={{ flexDirection:"column", alignItems:"flex-start", gap:8 }}>
            ...
            {competitionDate ? (
              <div style={{ display:"flex", gap:8, alignItems:"center", width:"100%" }}>
                ...
                <button onClick={clearCompetitionMode} className="ghost" style={{ padding:"8px 12px", fontSize:13 }}>X</button>
              </div>
            ) : showCompForm ? (
          ...
          {showChangePwd && (
            <form onSubmit={handleChangePwd} style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 10 }}>
              ...
              {pwdMsg && (
                <div className={!pwdMsg.startsWith("Error") ? "success-msg" : "login-error"}>
                  <Icon name={!pwdMsg.startsWith("Error") ? "CheckCircle" : "AlertCircle"} size={14} />
                  <span>{pwdMsg}</span>
                </div>
              )}
        ...
        <div className="settings-section" style={{ marginTop: 20 }}>
          ...
          <div style={{ background: "var(--panel2)", borderRadius: 14, padding: "14px 16px" }}>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: "var(--muted)", lineHeight: 1.5 }}>
              Traá tus entrenamientos desde <b style={{ color: "var(--text)" }}>Strong</b> o <b style={{ color: "var(--text)" }}>Hevy</b>.<br />
